...
def simulate_data_request(stakers, witnesses, attacker_nodes):
    ...
    for i in range(0, 4):
        for j, staker in enumerate(stakers):
            if random.random() < staker * witnesses * (2 ** i):
...
def main():
    ...
    for epoch in tqdm.tqdm(range(options.data_requests)):
        others_eligibility, attacker_eligiblity = simulate_data_request(stakers, options.witnesses, attacker_nodes)
    ...
    for i, (so, sa) in enumerate(zip(successful_data_requests, successful_data_request_attacks)):
        ...
        print(f"Successful data request attacks @ commit round {i + 1}: {successful_attack * 100:.2f}%")
